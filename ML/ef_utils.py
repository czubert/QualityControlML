...
def get_volume(volume):
    ...
    return st.number_input('', 0.0, 10000.0, volume, 10.0) / 1000
...
def get_active_surface_area(x, y):
    # # #
    # # Active surface
    #
    ...
    cols = st.columns(3)
    with cols[0]:
        active_x = st.number_input('First dimension (x)', 0.0, 50.0, x, 0.1)
    with cols[1]:
        active_y = st.number_input('Second dimension (y)', 0.0, 50.0, y, 0.1)
    with cols[2]:
